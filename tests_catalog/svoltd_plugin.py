...
class SVOLTDPlugin:
    ...
    def _click_continue_if_present(self, ctx, label, timeout=4):
        try:
            click_continue(ctx, label=f"SVOLTD {label}", timeout=timeout, delay=0.5)
            return True
        except Exception:
            return False
    ...
    def _click_continue_required(self, ctx, label, timeout=20):
        if self._response_elements_present(ctx):
            ctx.logger.info(f"SVOLTD {label}: response elements already present; skipping continue")
            return

        click_continue(ctx, label=f"SVOLTD {label}", timeout=timeout, delay=1)
    ...
    def _wait_for_response_buttons(self, ctx, timeout=15):
        ...
        if len(elements) < 4:
            raise RuntimeError(f"SVOLTD expected 4 response elements, found {len(elements)}")
    ...
    def _click_response(self, ctx, response_value):
        ...
        ctx.logger.info(
            "SVOLTD clicking response %s using element text=%r",
            response_value,
    ...
    def _main_test(self, ctx, strategy):
        ...
        while trial_count < self.TEST_TRIALS:
            ...
            ctx.logger.info(
                f"SVOLTD test trial {trial_count}/{self.TEST_TRIALS}: clicking response {response}"
            )
        ...
        ctx.logger.info(f"SVOLTD completed {trial_count} randomized test trials")

        if trial_count != self.TEST_TRIALS:
            raise RuntimeError(
                f"SVOLTD expected {self.TEST_TRIALS} test trials, completed {trial_count}"
            )
    ...
    def run(self, ctx, strategy="random"):
        ...
        try:
            ctx.logger.info("SVOLTD starting")
            ...
            ctx.logger.exception("SVOLTD failed")
            ctx.artifacts.capture_failure(ctx.driver, "svoltd_failure", {"errors": errors})
